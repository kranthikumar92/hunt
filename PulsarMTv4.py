...
def createParser ():
    ...
    return parser.parse_args().bip, parser.parse_args().database, parser.parse_args().threading, parser.parse_args().mode, \
        parser.parse_args().desc, parser.parse_args().bit, parser.parse_args().debug, parser.parse_args().mail, parser.parse_args().sleep

def run(bip, db_bf, mode, desc, bit, debug, mail, th, sleep,  counter, tr):
    inf.db_bf = db_bf
    inf.mode=mode
    email.desc=desc
    inf.bit=bit
...
if __name__ == "__main__":
    inf.bip, inf.db_bf, inf.th, inf.mode, email.desc, inf.bit, inf.debug, inf.mail, inf.sleep  = createParser()
    print('-'*70,end='\n')
    ...
    try:
        procs = [Process(target=run, name= str(i), args=(inf.bip, inf.db_bf, inf.mode, email.desc, inf.bit, inf.debug, inf.mail, inf.th, inf.sleep, counter, tr,)) for i in range(inf.th)]
    except KeyboardInterrupt:
        print('\n'+'Interrupted by the user.')
        sys.exit()
    try:
